...
A implementação garante que a chamada recursiva para exibir os elementos ocorra naturalmente.

```typescript
// Instanciando folhas genéricas
const titulo = new BlocoTexto("Como usar Padrões de Projeto", "titulo 1");
const codigo = new BlocoCodigo(
  "console.log('Padrão Composite rodando!');",
  "typescript",
);
...
Para aferir a pertinência e a qualidade da implementação, segue a gravação da execução do código (**versão 01**) rodando via terminal no ambiente de desenvolvimento: [Clique aqui para assistir ao vídeo da execução do BlockManager](https://youtu.be/vYR2FNTvt2I)
