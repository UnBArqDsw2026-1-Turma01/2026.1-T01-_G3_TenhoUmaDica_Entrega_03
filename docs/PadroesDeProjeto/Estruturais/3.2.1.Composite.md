...
```
#### Vídeos referente a implementação do diagrama BlockManager:
Para aferir a pertinência e a qualidade da implementação, segue a gravação dos vídeos da execução do código rodando via terminal no ambiente de desenvolvimento:  
[Clique aqui para assistir ao vídeo da execução do BlockManager (Versão 01)](https://youtu.be/vYR2FNTvt2I)  
[Clique aqui para assistir ao vídeo da execução do BlockManager (Versão 02)](https://youtu.be/HmpoyQcCz1s)
