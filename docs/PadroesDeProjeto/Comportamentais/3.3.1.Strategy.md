...
![Diagrama Strategy V2](../../assets/imagens/comportamentais/diagramaStrategyV2.jpg)

<font size="3">
<p>Fonte: Angélica, João Gabriel, Brenda e Marcos.</p>
</font>
...
### 6. OrdenacaoStrategyService — Orquestração

Coordena a seleção da estratégia e a execução via `Feed`. Mantém um registro das três estratégias disponíveis e instancia o `Feed` com a estratégia correta a cada requisição. É o único ponto do sistema que conhece as ConcreteStrategies existentes.
...
#### Service (`ordenacao-strategy.service.spec.ts`) — 12 testes
...
| `listarAlgoritmos` | retorna os três algoritmos; retorna exatamente 3 |
...
### 8. Evidência de Funcionamento via Postman
...
| 1.2 | Implementação código e correções da docuemtnação | Angélica | 22/05/2026 |
