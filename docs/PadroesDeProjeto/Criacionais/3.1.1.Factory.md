...
Os códigos completos referentes às estruturas do padrão Factory, interfaces e models utilizados no projeto podem ser encontrados no repositório oficial:

🔗 https://github.com/UnBArqDsw2026-1-Turma01/2026.1-T01-_G3_TenhoUmaDica_Entrega_03/tree/main/app/back/src/posts

### Observação


> Optamos por não inserir todos os códigos diretamente nesta documentação para evitar uma página excessivamente extensa e melhorar a visualização e organização do documento.

### Demonstração: Fábrica de Anúncios `teste-geral-posts`

O fluxo de criação segue o padrão **Factory Method**, garantindo a instância correta dos objetos de negócio. O terminal abaixo demonstra a execução do nosso teste de integração, validando a criação de todos os tipos de posts e a persistência no sistema.

![Diagrama Factory](../../assets/imagens/criacionais/GifTesteFactory.gif)

> **Destaque:** Observe a execução dos logs no terminal, que confirmam a renderização específica da classe `PostAnuncio`, validando os atributos de `fixado` e `comentariosBloqueados`.
...
| 1.2 | Adição codigos e teste | [João Gabriel](https://github.com/JoaoComTil) | 22/05/2026 |